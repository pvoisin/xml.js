(function(global) {
				...
				var reverters = {
					"#ELEMENT": function revert(object, document) {
						...
						while(index < children.length) {
							...
							var node = reverters[type](child, document);
						...
						for(var key in properties) {
							if(properties[key] !== children && key[0] !== "#") {
								var attribute = reverters["#ATTRIBUTE"](y.pick(properties, key), document);
					...
					"#ATTRIBUTE": function revert(object, document) {
						for(var name in object) break;
	...
	if(!AMD) {
		...
		var define = function define(modules, callback) {
